...
void MBSS_qstft_multi(double dv[32768],creal_T Cx[393984])
{
  ...
  memset(&Cx[0], 0, 393984U * sizeof(creal_T));
  ...
  for (f = 0; f < 513; f++) {
    ...
    for (t = 0; t < 3; t++) {
        ...
        for (nblocks = 0; nblocks < hi; nblocks++) {
          for (xblockoffset = 0; xblockoffset < indf_size_idx_1; xblockoffset++)
          {
            r->data[(xblockoffset + r->size[0] * nblocks) + r->size[0] * r->
              size[1] * lastBlockLength] = X[((indf_data[xblockoffset] + 513 *
              (y_data[nblocks] - 1)) + 1539 * lastBlockLength) - 1];
          }
        }
      }
      ...
      for (nblocks = 0; nblocks < 120; nblocks++) {
        ...
        Cx[(((nind + xblockoffset) + Cx_tmp) + 131328 * t) - 1] = local_Cx[(nind
          + xblockoffset) - 1];
        Cx[(((hi + xblockoffset) + Cx_tmp) + 131328 * t) - 1] = local_Cx[(hi +
          xblockoffset) - 1];
        xblockoffset = (hi - 1) << 4;
        Cx[(((nind + xblockoffset) + Cx_tmp) + 131328 * t) - 1] = local_Cx[(nind
          + xblockoffset) - 1];
