 ...
 *
 *
 * Arguments    : creal_T Cx[787200]
 * Return Type  : void
 */
void MBSS_qstft_multi(short dv[65536],creal_T Cx[787200])
{
  static creal_T X[49200];
  ...
  static const double winf[15] = { 0.038060233744356631, 0.14644660940672621,
    ...
    0.85355339059327373, 0.69134171618254481, 0.49999999999999994,
    0.30865828381745508, 0.14644660940672621, 0.038060233744356631 };

  static const double dv2[3] = { 0.49999999999999994, 1.0, 0.49999999999999994 };
  ...
  double b_tmp_data[3075];
  double tmp_data[1025];
  double pairId[240];
  double dv1[16];
  double b_data[3];
  double ai;
  ...
  for (f = 0; f < 1025; f++) {
    ...
    Cx_tmp = f << 8;
    for (lastBlockLength = 0; lastBlockLength < indf_size_idx_1; lastBlockLength
         ++) {
    ...
    for (t = 0; t < 3; t++) {
      ...
      for (lastBlockLength = 0; lastBlockLength < hi; lastBlockLength++) {
        b_data[lastBlockLength] = dv2[y_data[lastBlockLength] - t];
      }
      ...
      for (lastBlockLength = 0; lastBlockLength < nind; lastBlockLength++) {
        for (nblocks = 0; nblocks < 16; nblocks++) {
          wei->data[nblocks + 16 * lastBlockLength] = b_tmp_data[lastBlockLength];
        }
      }
      ...
      for (lastBlockLength = 0; lastBlockLength < 16; lastBlockLength++) {
        for (nblocks = 0; nblocks < hi; nblocks++) {
          for (xblockoffset = 0; xblockoffset < indf_size_idx_1; xblockoffset++)
          {
            r->data[(xblockoffset + r->size[0] * nblocks) + r->size[0] * r->
              size[1] * lastBlockLength] = X[((indf_data[xblockoffset] + 1025 *
              (y_data[nblocks] - 1)) + 3075 * lastBlockLength) - 1];
          }
        }
      }
      ...
      for (lastBlockLength = 0; lastBlockLength < nind; lastBlockLength++) {
        for (nblocks = 0; nblocks < 16; nblocks++) {
          XX->data[nblocks + 16 * lastBlockLength] = r->data[lastBlockLength +
            nind * nblocks];
        }
      }
      ...
      for (lastBlockLength = 0; lastBlockLength < xblockoffset; lastBlockLength
           ++) {
        b_XX->data[lastBlockLength].re = wei->data[lastBlockLength] * XX->
          data[lastBlockLength].re;
        b_XX->data[lastBlockLength].im = wei->data[lastBlockLength] * XX->
          data[lastBlockLength].im;
      }
      ...
      for (nblocks = 0; nblocks < 120; nblocks++) {
        ...
        Cx[(((nind + xblockoffset) + Cx_tmp) + 262400 * t) - 1] = local_Cx[(nind
          + xblockoffset) - 1];
        Cx[(((hi + xblockoffset) + Cx_tmp) + 262400 * t) - 1] = local_Cx[(hi +
          xblockoffset) - 1];
        xblockoffset = (hi - 1) << 4;
        Cx[(((nind + xblockoffset) + Cx_tmp) + 262400 * t) - 1] = local_Cx[(nind
          + xblockoffset) - 1];
