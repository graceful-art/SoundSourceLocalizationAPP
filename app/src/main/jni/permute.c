...
/*
 * Arguments    : const creal_T a[393216]
 *                creal_T b[393216]
 * Return Type  : void
 */
void permute(const creal_T a[393216], creal_T b[393216])
{
  int b_k;
  int c_k;
  int d_k;
  int k;
  for (k = 0; k < 3; k++) {
    for (b_k = 0; b_k < 512; b_k++) {
      for (c_k = 0; c_k < 16; c_k++) {
        for (d_k = 0; d_k < 16; d_k++) {
          b[((b_k + (k << 9)) + 1536 * d_k) + 24576 * c_k] =
              a[((d_k + (c_k << 4)) + (b_k << 8)) + (k << 17)];
        }
      }
    }
  }
