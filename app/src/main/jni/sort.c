...
/*
 * Arguments    : double x[32761]
 *                int idx[32761]
 * Return Type  : void
 */
void sort(double x[32761], int idx[32761])
{
  static double xwork[32761];
  double b_xwork[256];
  double x4[4];
  double d;
  double d1;
  ...
  x4[3] = 0.0;
  idx4[3] = 0;
  memset(&idx[0], 0, 32761U * sizeof(int));
  memset(&xwork[0], 0, 32761U * sizeof(double));
  nNaNs = 0;
  ib = 0;
  for (k = 0; k < 32761; k++) {
    if (rtIsNaN(x[k])) {
      idx[32760 - nNaNs] = k + 1;
      xwork[32760 - nNaNs] = x[k];
      nNaNs++;
    } else {
      ib++;
      idx4[ib - 1] = (short)(k + 1);
  ...
  memset(&iwork[0], 0, 32761U * sizeof(int));
  nNonNaN = 32761 - nNaNs;
  ib = 2;
  if (32761 - nNaNs > 1) {
    nBlocks = (32761 - nNaNs) >> 8;
    if (nBlocks > 0) {
      for (b = 0; b < nBlocks; b++) {
        i4 = (b << 8) - 1;
        for (b_b = 0; b_b < 6; b_b++) {
    ...
    merge_block(idx, x, 0, 32761 - nNaNs, ib, iwork, xwork);
  }
  if ((nNaNs > 0) && (32761 - nNaNs > 0)) {
    for (k = 0; k < nNaNs; k++) {
      ib = (k - nNaNs) + 32761;
      xwork[k] = x[ib];
      iwork[k] = idx[ib];
